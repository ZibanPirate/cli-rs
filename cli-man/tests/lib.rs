...
fn test_bin_path() {
    ...
    #[cfg(not(target_os = "windows"))]
    assert_eq!(installer.bin_path(), PathBuf::from(format!("{}/.cli-man/bin/testcli/1.0.0/testcli", home)));
    #[cfg(target_os = "windows")]
    assert_eq!(installer.bin_path(), PathBuf::from(format!("{}/.cli-man/bin/testcli/1.0.0/testcli.exe", home)));
}
